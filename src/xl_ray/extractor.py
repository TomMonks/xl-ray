...
def extract_worksheets(wb_formulas, wb_values) -> dict[str, WorksheetData]:
    ...
    # NOTE: Orphaned/terminal cell detection (bounding-box based "is_terminal"
    # check) has been removed for now. It was raising "too many values to
    # unpack (expected 2)" errors on some cell addresses passed to
    # coordinate_to_tuple(), and even when it ran cleanly it mislabelled the
    # vast majority of legitimate input/data cells as "orphaned" simply
    # because nothing referenced them via a formula we could parse. Revisit
    # this once "orphaned" is precisely defined (e.g. restricted to formula
    # cells with no dependents, excluding raw input cells, table/pivot/VBA
    # references, and named-range or cross-sheet references that the
    # tokenizer/regex parsing above can silently miss).
